...
* **FUNZIONALITÀ RICHIESTE**: Lettura di $N$, associazione di un indice progressivo e generazione iterativa dei punti interni del dominio determinando le coordinate $i, j, x, y$ della griglia e geometriche. Esportazione dei dati in un file `coords.txt`, generazione del grafo di adiacenza e creazione di un file `connectivity.txt` con la struttura $e$, $u$, $v$.
* **STRUTTURE DATI**: Utilizzo di `std::vector` per memorizzare i dati nella forma desiderata (si utilizza `using namespace std` per omettere `std::` nel codice) e di una `struct node`.
* **COMPLESSITÀ**: $O(N^2)$ – il tempo è lineare rispetto al numero totale di nodi, che sono $N \times N$.
* **DIPENDENZA DA ALTRI MODULI**: Genera il file `coords.txt` necessario per l'algoritmo di nested dissection (Parte 2). Inoltre, il file `connectivity.txt` contiene informazioni sui nodi ed è necessario per la Parte 3, al fine di identificare gli elementi fuori diagonale non nulli della matrice $A$.
* **DESCRIZIONE IMPLEMENTATIVA**: 
  ...
  La generazione dei nodi avviene tramite due cicli `for` annidati (interno per le $i$ ed esterno per le $j$), calcolando ad ogni iterazione le coordinate fisiche moltiplicando per il passo $h$. 
  Per la costruzione del grafo di adiacenza si utilizzano ulteriori cicli `for`: se un punto è a destra o in alto rispetto a un nodo $u$, viene inserito nei nodi adiacenti $v$ (non si controllano i vicini in basso o a sinistra per evitare doppie assegnazioni). I nodi $v$ vengono aggiunti alla lista di adiacenza di $u$ (e viceversa) tramite `push_back`. 
  Infine, l'esportazione in `coords.txt` avviene scorrendo tutti i nodi, mentre per `connectivity.txt` si utilizza un ciclo for sui nodi $u$ e uno interno sui relativi vicini $v$.
